...
import { Shop, User } from '../user/user.model';
import { USER_ROLES } from '../../../enums/user';

//login
const loginUserFromDB = async (payload: ILoginData) => {

    const { email, password, deviceToken,latitude,longitude } = payload;
    let isExistUser: any = await User.findOne({ email }).select('+password');
    if (!isExistUser) {
    ...
    if (
        ...
        await User.findOneAndUpdate(
            { email: email },
            { $set: { deviceToken: deviceToken, latitude: latitude||0, longitude: longitude||0 } },
            { new: true }
        )
    }
